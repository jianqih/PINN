...
class NN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, act_fn=nn.ReLU()):  # Changed from Tanh to ReLU
...
ax.set(title="Logistic equation solved with NNs (ReLU activation)", xlabel="t", ylabel="f(t)")
ax.legend()
# Save before showing
fig.savefig('pinn_task4_relu.png', dpi=300, bbox_inches='tight')
plt.show() 